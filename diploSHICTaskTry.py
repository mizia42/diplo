...
# Import a bunch of libraries if everything checks out
    ...
    from keras.layers import Conv2D, MaxPooling2D,concatenate,LSTM,TimeDistributed,Conv1D,MaxPooling1D,ConvLSTM2D
...
if argsDict['mode'] == 'train':
    ...
    for i in range(0,rows):
        loadList.append(dstack(h1[i]))
    h1=np.asarray(loadList)  
    print(h1.shape) 
    h1 = np.reshape(h1,(h1.shape[0],numSubWins,nDims))
    neut = np.loadtxt(trainingDir+"neut.fvec",skiprows=1)
    n1 = np.reshape(neut,(neut.shape[0],nDims,numSubWins))
    loadList=list()
    for i in range(0,rows):
        loadList.append(dstack(n1[i]))
    n1=np.asarray(loadList)  
    n1 = np.reshape(n1,(n1.shape[0],numSubWins,nDims))
    soft = np.loadtxt(trainingDir+"soft.fvec",skiprows=1)
    s1 = np.reshape(soft,(soft.shape[0],nDims,numSubWins))
    loadList=list()
    for i in range(0,rows):
        loadList.append(dstack(s1[i]))
    s1=np.asarray(loadList)  
    s1 = np.reshape(s1,(s1.shape[0],numSubWins,nDims))
    lsoft = np.loadtxt(trainingDir+"linkedSoft.fvec",skiprows=1)
    ls1 = np.reshape(lsoft,(lsoft.shape[0],nDims,numSubWins))
    loadList=list()
    for i in range(0,rows):
        loadList.append(dstack(ls1[i]))
    ls1=np.asarray(loadList)  
    ls1 = np.reshape(ls1,(ls1.shape[0],numSubWins,nDims))
    lhard = np.loadtxt(trainingDir+"linkedHard.fvec",skiprows=1)
    lh1 = np.reshape(lhard,(lhard.shape[0],nDims,numSubWins))
    loadList=list()
    for i in range(0,rows):
        loadList.append(dstack(lh1[i]))
    lh1=np.asarray(loadList)  
    lh1 = np.reshape(lh1,(lh1.shape[0],numSubWins,nDims))
   ...
   # both = both.reshape(both.shape[0],nDims,numSubWins)
    if (trainingDir==testingDir):
        print("comes in if part")
        ...
        lh1 = np.reshape(lh1,(lh1.shape[0],numSubWins,nDims))
        # hard = np.loadtxt(testingDir+"hard.fvec",skiprows=1)
        # h1 = np.reshape(hard,(hard.shape[0],numSubWins,nDims))
        # neut = np.loadtxt(testingDir+"neut.fvec",skiprows=1)
        # n1 = np.reshape(neut,(neut.shape[0],numSubWins,nDims))
        # soft = np.loadtxt(testingDir+"soft.fvec",skiprows=1)
        # s1 = np.reshape(soft,(soft.shape[0],numSubWins,nDims))
        # lsoft = np.loadtxt(testingDir+"linkedSoft.fvec",skiprows=1)
        # ls1 = np.reshape(lsoft,(lsoft.shape[0],numSubWins,nDims))
        # lhard = np.loadtxt(testingDir+"linkedHard.fvec",skiprows=1)
        # lh1 = np.reshape(lhard,(lhard.shape[0],numSubWins,nDims))

        both2=np.concatenate((h1,n1,s1,ls1,lh1))
        X_test = both2
        #.reshape(both2.shape[0],numSubWins,nDims,)
    ...
    #save_img('bondi_beach_grayscale.jpg', X_train[0])
    ...
    #print(X_train.shape)
    print(X_train.shape)
    # print("validation set has %d examples" % X_valid.shape)
    # print("test set has %d examples" % X_test.shape)
    n_steps, n_length = 1,11
    n_features=X_train.shape[2] #12
    n_timesteps=X_train.shape[1] #11
    ...
    # model.add(ConvLSTM2D(filters=64, kernel_size=(1,3), activation='relu', input_shape=(n_steps, 1, n_length, n_features),go_backwards=True))
    # model.add(Dropout(0.5))
    # model.add(Flatten())
    # model.add(Dense(100, activation='relu'))
    # model.add(Dense(5, activation='softmax'))

    model.add(ConvLSTM2D(filters=226,kernel_size=(1,3), activation='relu', input_shape=(n_steps, 1, n_length, n_features)))
    model.add(Dropout(0.5))
    model.add(Flatten())
    model.add(Dense(100, activation='relu'))
    model.add(Dense(5, activation='softmax'))
    ...
    # h = ConvLSTM2D(filters=64, kernel_size=(1,3), activation='relu', input_shape=(n_steps, 1, n_length, n_features))(model_in)
    # h = Dropout(0.5, name='drop1')(h)
    # h= Flatten(name='dflaten1')(h)
    
    # dh = ConvLSTM2D(filters=64, kernel_size=(1,3), activation='relu', input_shape=(n_steps, 1, n_length, n_features))(model_in)
    # dh = Dropout(0.5, name='drop12')(dh)
    # dh= Flatten(name='dflaten12')(dh)

    # dh1 = ConvLSTM2D(filters=64, kernel_size=(1,3), activation='relu', input_shape=(n_steps, 1, n_length, n_features))(model_in)
    # dh1 = Dropout(0.5, name='drop13')(dh1)
    # dh1= Flatten(name='dflaten13')(dh1)
    ...
    # h = Dense(100,name="512dense",activation='relu')(h)
    # output = Dense(5,name="out_dense",activation='softmax')(h)
    # model = Model(inputs=[model_in], outputs=[output])
    time_callback = TimeHistory()
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.fit(X_train, Y_train, epochs=10, batch_size=32, verbose=0,callbacks=[time_callback])
    ...
    #   batch_size=32, epochs=epochOption, verbose=1)
    # print(history)
    ...
    #                     validation_steps=len(X_test)/32,shuffle=False)
    # X_new=X_test
    # Y_new=Y_test
    # #Sprint(history)
    # #model.fit(X_train, Y_train, batch_size=32, epochs=100,validation_data=(X_test,Y_test),callbacks=callbacks_list, verbose=1)
    # score = model.evaluate_generator(test_gen.flow(X_test,Y_test, batch_size=32,shuffle=False),len(Y_test)/32, pickle_safe=True)
    ...
    # # new changes Artimizia
    from sklearn.metrics import confusion_matrix,accuracy_score

    # generator = datagen.flow(
    #     X_test, Y_test, batch_size=32)  # keep data in same order as labels
    # print("inputs are ")
    # print(X_test.shape)
    # print(Y_test.shape)

    Y_pred = model.predict(X_test)
    # Y_pred = model.predict_generator(test_gen.flow(X_new,Y_new, batch_size=32,shuffle=False),len(Y_valid)/32)
    # #y_batch, y_class = test_gen[0] 
    # print("bfr argmax")
    # print(Y_new.shape)
    # print(Y_pred.shape)
    ...
    print('chrom\tclassifiedWinStart\tclassifiedWinEnd\tbigWinRange\tpredClass\tprob(neutral)\tprob(likedSoft)\tprob(linkedHard)\tprob(soft)\tprob(hard)\n')
